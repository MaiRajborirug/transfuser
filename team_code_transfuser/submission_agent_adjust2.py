"""
ad CBF and remove savety box in the original control
"""
import os
import json
...
class HybridAgent(autonomous_agent.AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp):
        ...
        self.lidar_std = 0.0
        sigma_i = np.random.uniform(-self.lidar_std, self.lidar_std, N)
        exp_sigma_i = np.exp(sigma_i).reshape(-1, 1)
        input_data['lidar'][1][:, :3] = lidar_temp * exp_sigma_i
        ...
        with torch.no_grad():
            ...
            for i in range(self.model_count):
                ...
                if (self.backbone == 'transFuser'):
                    ...
                    pred_wp, rotated_bb = self.nets[i].forward_ego(image, lidar_bev, target_point, target_point_image, velocity,
                                                          num_points=num_points, save_path=SAVE_PATH, stuck_detector=self.stuck_detector,
                                                          forced_move=is_stuck, debug=self.config.debug, rgb_back=self.rgb_back)
                    # NOTE: added
        ...
        print('bb_buffer', self.bb_buffer)
        """ detect distance and perform emergency stop"""
        min_x = 6
        emergen_break = False
        for box_i in self.bb_buffer[0]: # (6,3)
            # distance front
            for j in range(len(box_i)):
                if box_i[j, 0] < min_x:
                    distance_front = box_i[j, 0]
                    emergen_break = True
                    break
        ...
        """always set norminal control brake to 0 and nominal control throttle to +0.3"""
        control.brake = False
        if emergen_break:
            control.throttle = 0.0
            control.brake = True
        ...
        """remove safety boxes in the original control"""
