# pure transfuser with visualization
import os
import json
import datetime
import pathlib
...
# from transfuser.team_code_transfuser.visualize_optical_flow import OpticalFlowVisualizer
...
class agent(HybridAgent):
    def setup(self, path_to_conf_file):
        ...
        # relate to saving data
        self.step = -1
        self.frame_rate_sim = 20 # CARLA framerate, used for visualizations
        self.save_freq = 2 # save every self.save_freq / self.frame_rate_sim seconds
        if self.save_path is not None:
            now = datetime.datetime.now()
            string = Path(os.environ['ROUTES']).stem + '_'
            string += f'route{self.route_index}_'
            string += '_'.join(map(lambda x: '%02d' % x, (now.month, now.day, now.hour, now.minute, now.second)))
            self.save_path = Path(os.environ['SAVE_PATH']) / string
            self.save_path.mkdir(parents=True, exist_ok=False)
            
            # (self.save_path).mkdir() # global map
            (self.save_path / 'topdown').mkdir() # global map
    ...
    def sensors(self):
        ...
        sensors += [
                    # NOTE: add GT + visualization
                    {
                        'type': 'sensor.camera.rgb',
                        'x': 0, 'y': 0, 'z':60,
                        'roll': 0, 'pitch': -90, 'yaw': 0,
                        'width': 800, 'height': 800, 'fov': 70,
                        'id': 'rgb_bev'
                    },                   
                    {
    ...
    # NOTE: adjust crop and scale function
    def scale_crop(self, image, scale=1, start_x=0, crop_x=None, start_y=0, crop_y=None):
        ...
        cropped_image = image[:, start_x:start_x+crop_x] # keep y the same
        return cropped_image

    # NOTE: added
    def _prepare_image(self, input_data):
        """have depth and semantics as well"""
        rgb = []
        semantics = []
        depth = []
        for pos in ['left', 'front', 'right']:
            ...
            semantics_pos = input_data[semantics_cam][1][:, :, :3]
            semantics_pos = self.scale_crop(Image.fromarray(semantics_pos), self.config.scale, self.config.img_width, self.config.img_width, self.config.img_resolution[0], self.config.img_resolution[0])
            semantics.append(semantics_pos)
            
            depth_cam = 'depth_' + pos
            depth_pos = input_data[depth_cam][1][:, :, :3]
        ...
        return (rgb.astype(np.uint8), semantics.astype(np.uint8), depth.astype(np.uint8))
    ...
    def run_step(self, input_data, timestamp):
        ...
        (self.rgb, self.segmentic, self.depth) = self._prepare_image(input_data)
        self.segmentation.vis_gt(self.rgb, self.segmentic)  # visualize input + semantic GT
        ...
        # NOTE METHOD 3 merge and use GT for segmentation
        ...
        data = {
            # relate to vehicle. get_transform().get_matrix()
            'x_loc': input_data['gps'][1][3],
            'y_loc': input_data['gps'][1][4],
            'z_loc': input_data['gps'][1][5],
            'roll': input_data['gps'][1][6],
            'pitch': input_data['gps'][1][7],
            'yaw': input_data['gps'][1][8],
            # 'y': pos[1],
            'gps': list(input_data['gps'][1][:2]), # turn to list -> save-able in Json
            'theta': input_data['imu'][1][-1] ,
            'speed': input_data['speed'][1]['speed'],
            'acceleration_x': input_data['imu'][1][0],
            'acceleration_y': input_data['imu'][1][1],
            'acceleration_z': input_data['imu'][1][2],
            'gyroscope_x': input_data['imu'][1][3],
            'gyroscope_y': input_data['imu'][1][4],
            'gyroscope_z': input_data['imu'][1][5],
            # 'target_speed': target_speed,
            # 'x_command': far_node[0],
            # 'y_command': far_node[1],
            # 'command': self.commands[-2],
            # 'waypoints': waypoints,
            'steer': transfuser_out.steer,
            'throttle': transfuser_out.throttle,
            'brake': transfuser_out.brake,
        ...
        self.save_sensors(input_data, data)
        return transfuser_out
    ...
    def save_sensors(self, input_data, data):
        ...
        cv.imwrite(str(self.save_path / 'rgb' / ('%04d.png' % frame)), bgr)
        cv.imwrite(str(self.save_path / 'rgb_bev' / ('%04d.png' % frame)), input_data['rgb_bev'][1])
        ...
        cv.imwrite(str(self.save_path / 'semantics' / ('%04d.png' % frame)), self.segmentic)
        cv.imwrite(str(self.save_path / 'depth' / ('%04d.png' % frame)), self.depth)

        np.save(self.save_path / 'lidar' / ('%04d.npy' % frame), input_data['lidar'], allow_pickle=True)
        np.save(self.save_path / 'lidar_semantic' / ('%04d.npy' % frame), input_data['lidar_semantic'], allow_pickle=True)
