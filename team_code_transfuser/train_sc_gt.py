...
from data import CARLA_Data, lidar_bev_cam_correspondences, CARLA_Data_new # dataloader
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=35, help='Number of train epochs.')
    parser.add_argument('--lr', type=float, default=2.5e-5, help='Learning rate.') # e-6 now
    ...
    parser.add_argument('--logdir', type=str, default='/media/haoming/970EVO/Pharuj/transfuser_training', help='Directory to log data to.')
    parser.add_argument('--load_file', type=str, default=None, help='ckpt to load.')
    # parser.add_argument('--load_file', type=str, default='/home/haoming/git/transfuser/training/log_train_nov/train_scratch/model_31.pth', help='ckpt to load.')
    ...
    parser.add_argument('--arg_file', type=str, default='/home/haoming/git/transfuser/model_ckpt/models_2023/args_transfuser_from0.txt', help='Path to a txt file containing the arguments. The arguments in the file will overwrite the arguments passed via the command line.')
...
class Engine(object):
    ...
    def __init__(self, model, optimizer, dataloader_train, dataloader_val, args, config, writer, device, rank=0, world_size=1, parallel=False, cur_epoch=0):
        ...
        # NOTE: initiate brake_sc CrossEntropy Loss
        self.criterionCE = torch.nn.CrossEntropyLoss()  # criterion(steers_p, steer_sc)
        ...
        self.segmentation = segmentation() # Note: case 2 use segmentation
    ...
    def train(self):
        ...
        for data in tqdm(self.dataloader_train):
            ...
            # NOTE: case 1 use gt segmentaion instead of prediction
            semantic_ = data['semantic'].squeeze(1) # [batch, 160, 704]
            semantic_seg = semantic_[:,:,rgb_w-self.config.img_width//2:rgb_w+self.config.img_width//2]
            is_road, _, _, is_animated = self.segmentation.predict_gt_batch(semantic_seg)
            
            # NOTE: case 2 use prediction model 
            ...
            for i in range(is_road.shape[0]): # ~batch size
                ...
                v_e = np.max([0.05, speed[i]]).astype(np.float32) # set v_e >= 0
                ...
                if throttle_i > 0:
                    ...
                    # interfuser_acc = np.maximum(interfuser_acc*0.3,0) # damp down a a bit
                else:
                    interfuser_acc = -13
            ...
            loss_throttle_sc = self.criterionL1(torch.tensor(throttles_p, dtype=torch.float), torch.tensor(throttle_sc, dtype=torch.float)) # torch.mean(torch.abs(torch.tensor(throttles_p) - torch.tensor(throttle_sc)))
            loss_steer_sc = self.criterionL1(torch.tensor(steers_p, dtype=torch.float), torch.tensor(steer_sc, dtype=torch.float)) # torch.mean(torch.abs(torch.tensor(steers_p) - torch.tensor(steer_sc)))
            loss_brake_sc = self.criterionCE(torch.tensor(brake_p, dtype=torch.float), torch.tensor(brake_sc, dtype=torch.float))
            losses.update({
                'throttle_sc': loss_throttle_sc,
                'steer_sc': loss_steer_sc,
                'brake_sc': loss_brake_sc,
            }) # add sc loss in the dict
