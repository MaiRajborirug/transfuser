...
# find paddleseg
try:
    sys.path.append("/home/haoming/PaddleSeg/deploy/python")
    from paddleseg_carla_2 import Predictor
except ImportError:
    raise RuntimeError("cannot import paddleseg")
...
class KeyboardControl(object):
    ...
    def parse_events(self, client, world, clock):
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                is_testing = self._parse_vehicle_keys(pygame.key.get_pressed(), clock.get_time())
            ...
            if world.constant_velocity_enabled:
                is_testing = False
            return False, self._control, is_testing
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        vehicles = world.world.get_actors().filter('vehicle.lincoln.mkz2017')
        self._info_text = [
...
class optical_flow:
    ...
    def frame_cb(self, raw):
        frame = self.unflatten(raw)
        ...
        print("before adding opfical flow viz")
        print(self.hsv.shape)
...
def game_loop(args):
    ...
    measurement_file = open('a_-10.txt', 'w')
    ...
    try:
        ...
        # cam_bp.set_attribute("image_size_x",str(1000))
        # cam_bp.set_attribute("image_size_y",str(500))
        ...
        # # set up optical flow
        # flow = optical_flow()
        # seg = Segmentation() # todo: rename the segmentation class
        # ego_cam.listen(lambda image: seg.cb(np.asarray(image.raw_data)))
        ...
        while True:
            ...
            done, control, is_testing = controller.parse_events(client, world, clock)
            player = world.player
            v = player.get_velocity()
            speed = math.sqrt(v.x**2 + v.y**2 + v.z**2) # in m/s
            a = world.imu_sensor.accelerometer
            acceleration = math.sqrt(a[0]**2 + a[1]**2) # in m2/s
            angular_v = world.imu_sensor.accelerometer[2]  # rad/s
            t = time.time()
            dt = t - last_t
            last_t = t
            angular_a = (angular_v - last_angular_v) / dt
            last_angular_v = angular_v

            if is_testing:
                t = control.throttle
                b = control.brake
                s = control.steer
                data = "{},{},{}\n".format(b, speed, acceleration)
                measurement_file.write(data)
                # print(data)
