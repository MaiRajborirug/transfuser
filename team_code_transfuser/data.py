...
class CARLA_Data_new(Dataset):
    ...
    def __init__(self, root, config, shared_dict=None):
        ...
        self.images_sf = [] # NOTE added: image of the second frame
        self.images = []
        self.bevs = []
        self.depths = []
        self.semantics_sf = [] # NOTE added: semantic of the second frame
        self.semantics = []
        self.lidars = []
        self.labels = []
        self.measurements = []
        ...
        for sub_root in tqdm(root, file=sys.stdout):
            ...
            for route in routes:
                ...
                for seq in range(2, num_seq - self.pred_len - self.seq_len - 2):
                    ...
                    image_sf = [] # NOTE added: image of the second frame
                    image = []
                    bev = []
                    depth = []
                    semantic_sf = [] # NOTE added: semantic of the second frame
                    semantic = []
                    lidar = []
                    label = []
                    measurement= []
                    ...
                    for idx in range(self.seq_len):
                        image_sf.append(route_dir / "rgb" / ("%04d.png" % (seq + idx + 1))) # NOTE added
                        ...
                        semantic_sf.append(route_dir / "semantics" / ("%04d.png" % (seq + idx + 1))) # NOTE added
                    ...
                    self.images_sf.append(image_sf) # NOTE added:
                    self.images.append(image)
                    self.bevs.append(bev)
                    self.depths.append(depth)
                    self.semantics_sf.append(semantic_sf) # NOTE added:
        ...
        self.images_sf    = np.array(self.images_sf   ).astype(np.string_)
        ...
        self.semantics_sf = np.array(self.semantics_sf).astype(np.string_)
    ...
    def __getitem__(self, index):
        ...
        images_sf = self.images_sf[index] # NOTE added
        images = self.images[index]
        bevs = self.bevs[index]
        depths = self.depths[index]
        semantics_sf = self.semantics_sf[index] # NOTE added
        ...
        # TODO: adjust self.data_cache to also cache the images_sf
        for i in range(self.seq_len):
            if not self.data_cache is None and str(measurements[i], encoding='utf-8') in self.data_cache:
                    measurements_i, images_sf_i, images_i, lidars_i, lidars_raw_i, bevs_i, depths_i, semantics_sf_i, semantics_i = self.data_cache[str(measurements[i], encoding='utf-8')] # json
                    images_sf_i = cv2.imdecode(images_sf_i, cv2.IMREAD_COLOR) # NOTE added
                    images_i = cv2.imdecode(images_i, cv2.IMREAD_UNCHANGED)
                    depths_i = cv2.imdecode(depths_i, cv2.IMREAD_UNCHANGED)
                    semantics_sf_i = cv2.imdecode(semantics_sf_i, cv2.IMREAD_UNCHANGED)  # NOTE added
                ...
                images_sf_i = cv2.imread(str(images_sf[i], encoding='utf-8'), cv2.IMREAD_COLOR) # NOTE added
                if(images_sf_i is None):
                    print("Error loading file (images_sf): ", str(images_sf[i], encoding='utf-8'))
                images_sf_i = scale_image_cv2(cv2.cvtColor(images_sf_i, cv2.COLOR_BGR2RGB), self.scale)
                ...
                if self.multitask:
                    ...
                    semantics_sf_i = cv2.imread(str(semantics_sf[i], encoding='utf-8'), cv2.IMREAD_UNCHANGED) # NOTE added
                    if (semantics_sf_i is None):
                        print("Error loading file: ", str(semantics_sf[i], encoding='utf-8'))
                    semantics_sf_i = scale_seg(semantics_sf_i, self.scale)
                ...
                if not self.data_cache is None:
                    ...
                    result, compressed_imgage_sf = cv2.imencode('.png', images_sf_i) # NOTE: added
                    result, compressed_imgage = cv2.imencode('.png', images_i)
                    result, compressed_depths = cv2.imencode('.png', depths_i)
                    result, compressed_semantics_sf = cv2.imencode('.png', semantics_sf_i) # NOTE: added
                    ...
                    self.data_cache[str(measurements[i], encoding='utf-8')] = (measurements_i, compressed_imgage_sf, compressed_imgage, lidars_i, lidars_raw_i, compressed_bevs, compressed_depths,compressed_semantics_sf, compressed_semantics)
                    # NOTE added

            loaded_images_sf.append(images_sf_i)
            loaded_images.append(images_i)
            loaded_bevs.append(bevs_i)
            loaded_depths.append(depths_i)
            loaded_semantics_sf.append(semantics_sf_i)
        ...
        images_sf_i = loaded_images_sf[self.seq_len-1] # NOTE: added
        images_sf_i = crop_image_cv2(images_sf_i, crop=self.img_resolution, crop_shift=crop_shift)
        ...
        data['rgb_sf'] = images_sf_i # NOTE: added
        data['rgb'] = images_i
        data['bev'] = bevs_i
        ...
        if self.multitask:
            ...
            semantics_sf_i = loaded_semantics_sf[self.seq_len-1] # NOTE: added
            semantics_sf_i = self.converter[crop_seg(semantics_sf_i, crop=self.img_resolution, crop_shift=crop_shift)]
            ...
            data['depth'] = depths_i 
            data['semantic_sf'] = semantics_sf_i # NOTE: added
