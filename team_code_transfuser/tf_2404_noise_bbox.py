...
class HybridAgent(autonomous_agent.AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp):
        ...
        # NOTE: intervine the control signal
        """
        U_tr = Transfuser_control = steer, throttle, brake
        
        S = state 
        
        def condition(S, U) = > True / False
        
        1. if condition(S, U) == True: -> return U_tr
        2. if condition(S, U) == False: ->  
            U_safe = argmin U' ||U'-U_tr||^2_2 s.t. condition(S, U') == True  # multiple U'
            
        # implement step 2:
        U_tr
        discretize choises of U'
        acc = 0 to 1 -> candidate_acc = [0, 0.1,...,1.0]
        steer = -1 to 1 -> candidate_steer =  [-1, -0.9,...,1.0]
        
        candidate_U = [(0,-1), (0,-0.9),...,(1,1)] # candidate control to test
        
        sort candidate_U by ||U'-U_tr||^2_2  # U' that close to U_tr comes first
        
        for U' in candidate_U:
            if condition(S, U') == True:
                return U'
            else no U' is safe:
                return U' = U_break
        
        # put the output back to carla
        control.steer = U'.steer
        control.throttle = U'.throttle
        control.brake = U'.brake    
        """
