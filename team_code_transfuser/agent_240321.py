...
class agent(HybridAgent):
    def setup(self, path_to_conf_file):
        ...
        self._set_certificate_bound(x_deg_close=31, x_deg_far=13, y_deg=12, decay_temp=1.09, offset_scale=0.002)
        self.certify_threshold = 0.992
        ...
        # NOTE: for semantic segmentation
    ...
    def run_step(self, input_data, timestamp):
        ...
        # TODO: now the variable name is transfuser --> suppose to be transfuser
        ...
        if res is None:
            ...
            if abs(a_control) < 0.7 * abs(phi_control) * self.turning_radius: # prioritize turning arbitary comparison
                # print('sc case 1')
                # throttle mapping
            ...
            elif a_control > -0.3: # prioritize throttle
                # print('sc case 2')
                # throttle mapping           
                delta = np.clip(math.sqrt(v_e) * self.c_speed_sqrt + a_control * self.c_acc + self.w_e**2 * self.c_w_sq + abs(self.w_e) * self.c_w, 0.0, 0.25)
                throttle = self.throttle_controller.step(delta)
    ...
    def _calculate_steer(self, speed, imu_omega_z):
        ...
        steer = c * math.atan(math.sqrt(wheel_length * wheel_length / (R * R - COM_length * COM_length))) * np.sign(imu_omega_z)
        np.clip(steer,-1,1)
        return steer
