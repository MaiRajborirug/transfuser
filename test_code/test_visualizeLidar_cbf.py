...
def update(frame):
    ...
    mask_satisfy = ((_h_t(X, Y, R_i, v_i, dt, gamma, v_e, a_e, R_e) - (1*gamma)*_h(X, Y, R_i))>0) | (Y<0)
    mask_not_satisfy = mask_satisfy ==0
    # print('lowest certify:', np.min(certify))
    
    #-------------

    # Define the size and color conditionally
    # sizes = np.where(colors==1, 3, 1)  # Larger size for certify < 0
    # colors = np.where(certify < 0, 'r', certify)  # Red color for certify < 0, otherwise use 'certify' for color scale

    scatter_satisfy = ax.scatter(X[mask_satisfy], Y[mask_satisfy], Z[mask_satisfy], c='g', label="Satisfy CBF (1)", alpha=0.8, s=1)
    scatter_not_satisfy = ax.scatter(X[mask_not_satisfy], Y[mask_not_satisfy], Z[mask_not_satisfy], c='r', label="Not satisfy CBF (0)", alpha=0.8, s=3)
    scatter_car = ax.scatter([0],[0],[0], c='black', label="Car", marker='>', s=40)
    # # If scatter is None, initialize the scatter plot and color bar
    # if scatter is None:
 
    #     # scatter = ax.scatter(X, Y, Z, c=colors, cmap='viridis', alpha=0.8, s=sizes)  # Increase point size (s) for visibility
    #     # color_bar = fig.colorbar(scatter, ax=ax, label='1=safe 0=risky')
    # else:
    #     # Update the scatter plot without clearing the axis
    #     scatter = ax.scatter(X, Y, Z, c=colors, cmap='viridis', alpha=0.8, s=1)  # Increase point size (s) for visibility
    ...
    # Set the axis limits
    ax.set_xlim([-80, 80])  # X-axis range
    ax.set_ylim([-80, 80])   # Y-axis range
