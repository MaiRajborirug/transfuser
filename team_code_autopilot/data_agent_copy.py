...
class DataAgent(AutoPilot):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        if self.save_path is not None:
            (self.save_path / 'rgb_front').mkdir()  # added
            (self.save_path / 'depth_front').mkdir()
            (self.save_path / 'semantics_front').mkdir()
        ...
        if self.save_path is not None:
            result += [
                    # added
                    {
                        ...
                        'width': self.largecam_config['width'], 'height': self.largecam_config['height'], 'fov': self.largecam_config['fov'],
                        'id': 'rgb_f'
                    },
                    {
    ...
    def tick(self, input_data):
        ...
        if self.save_path is not None:
            ...
            semantics_img = input_data['semantics_f'][1][:, :, 2]
            depth_img = input_data[depth_cam][1][:, :, :3]
            _semantics = np.copy(semantics_img)
            _depth = self._get_depth(depth_img)
            self._change_seg_tl(_semantics, _depth, self._active_traffic_light)

            result.update({'rgb_front': input_data['rgb_f'][1][:, :, :3], # added
                           'semantics_front': _semantics, # added
                           'depth_front': cv2.cvtColor(input_data['depth_f'][1][:, :, :3], cv2.COLOR_BGR2RGB), # added
                            'lidar': lidar,
                            'rgb': rgb,
                            'cars': cars,
                            'semantics': semantics,
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.step % self.save_freq == 0:
            if self.save_path is not None:
                tick_data = self.tick(input_data) # input from tick
    ...
    def shuffle_weather(self):
        ...
        # dtime, altitude = list(self.daytimes.items())[0]
    ...
    def save_sensors(self, tick_data):
        ...
        # added
        img = tick_data['rgb_front']  # already in BGR format
        cv2.imwrite(str(self.save_path / 'rgb_front' / ('%04d.png' % frame)), img)
        
        img = tick_data['depth_front']  # already in BGR format
        cv2.imwrite(str(self.save_path / 'depth_front' / ('%04d.png' % frame)), img)
        
        img = tick_data['semantics_front']  # already in BGR format
        cv2.imwrite(str(self.save_path / 'semantics_front' / ('%04d.png' % frame)), img)
