...
import sys
import os
sys.path.append('/media/haoming/970EVO/Pharuj/git/transfuser/team_code_transfuser')
sys.path.append('/media/haoming/970EVO/Pharuj/git/transfuser')
...
class DataAgent(AutoPilot):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        if self.save_path is not None:
            (self.save_path / 'rgb_seg').mkdir()  # added
    ...
    def tick(self, input_data):
        ...
        if self.save_path is not None:
            ...
            lidar_semantic = input_data['lidar_semantic']
            cars = self.get_bev_cars(lidar=lidar)
            
            # add acc
            accelerometer_x = input_data['imu'][1][0]
            accelerometer_y = input_data['imu'][1][1]
            accelerometer_z = input_data['imu'][1][2]
            gyroscope_x = input_data['imu'][1][3]
            gyroscope_y = input_data['imu'][1][4]
            gyroscope_z = input_data['imu'][1][5]
            

            result.update({'rgb_seg': rgb_seg, # added
                            'lidar': lidar,
                            'rgb': rgb,
                            'cars': cars,
                            'semantics': semantics,
    ...
    def save_sensors(self, tick_data):
        ...
        # added
        img = tick_data['rgb_seg']  # already in BGR format
        cv2.imwrite(str(self.save_path / 'rgb_seg' / ('%04d.png' % frame)), img)
